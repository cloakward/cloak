...
pub struct SessionRecord {
    ...
    /// issuance. The daemon's per-connection pidfd watcher passes
    /// this value to [`SessionStore::revoke_by_pidfd_inode`] the
    /// moment the peer task exits, closing the PID-recycle window.
    /// `None` on macOS / when no pidfd was available.
