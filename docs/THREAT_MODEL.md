...
| **A8 — PID recycle attack** | Reuse a freed PID to impersonate a trusted peer | **macOS:** the session token is bound to the connection's audit-token; if the connection's peer process exits, the kqueue `EVFILT_PROC` watcher invalidates the token before a new PID can recycle in. **Linux:** at handshake the daemon opens a kernel `pidfd` for the peer (`SO_PEERPIDFD` on 6.5+, falling back to `pidfd_open(SO_PEERCRED.pid)`), records its `st_ino` in the session record, and runs a `tokio::io::unix::AsyncFd` watcher per connection. The instant the kernel signals `POLLIN` on the pidfd (the peer task has exited), the watcher revokes every session bound to that pidfd inode and tears down the connection — closing the PID-reuse window. |
